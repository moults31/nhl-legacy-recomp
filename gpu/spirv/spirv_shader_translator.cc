 ...
 ******************************************************************************
 *
 * @modified  high-cut path C, P-2b - ported to ReXGlue/rex:: (namespace + include
 *            remap; bodies otherwise unchanged from Xenia 95a5c3e, reconciled where
 *            the SDK kVersion-12 spirv_translator.h header forced signature/member
 *            fixes). kVersion-12-only features (tessellation, clip planes,
 *            draw-resolution-scale, memexport-compute, float24 depth modes) are NOT
 *            implemented by these upstream kVersion-6 bodies. See
...
std::vector<uint8_t> SpirvShaderTranslator::CreateDepthOnlyFragmentShader(
  ...
  // P-2b: this kVersion-6 body ignores depth_stencil_mode (the float24 depth
  // modes are a kVersion-12-only feature not implemented by this upstream port).
...
void SpirvShaderTranslator::Reset() {
  ...
  for (size_t memexport_eM_index = 0;
       memexport_eM_index < rex::countof(var_main_memexport_data_);
       ++memexport_eM_index) {
...
void SpirvShaderTranslator::StartTranslation() {
  ...
  function_main_ = builder_->makeFunctionEntry(
      spv::NoPrecision, type_void_, "main", spv::LinkageTypeMax, main_param_types,
  ...
  if (!is_depth_only_fragment_shader_) {
    ...
    if (memexport_used) {
      ...
      while (
          rex::bit_scan_forward(memexport_eM_remaining, &memexport_eM_index)) {
...
void SpirvShaderTranslator::StartFragmentShaderBeforeMain() {
  ...
  if (!is_depth_only_fragment_shader_) {
    ...
    // Interpolator inputs.
    {
      ...
      uint32_t interpolator_index;
      while (
          rex::bit_scan_forward(interpolators_remaining, &interpolator_index)) {
        interpolators_remaining &= ~(UINT32_C(1) << interpolator_index);
        spv::Id interpolator = builder_->createVariable(
  ...
  if (!is_depth_only_fragment_shader_) {
    ...
    if (!edram_fragment_shader_interlock_) {
      ...
      while (
          rex::bit_scan_forward(color_targets_remaining, &color_target_index)) {
...
void SpirvShaderTranslator::StoreResult(const InstructionResult& result,
  ...
  if (value_num_components > 1) {
    ...
    while (rex::bit_scan_forward(used_result_components_remaining,
                                &result_component)) {
