...
REM
REM Stage 1 (this script): Release (-O3 -DNDEBUG, drops -g + asserts) + -march=x86-64-v3
REM   (AVX2/BMI/MOVBE codegen for the recomp's byte-swap/FP hot paths). NOTE: use a
REM   PORTABLE arch, NOT -march=native. native on an AMD dev box enables SSE4a, and
REM   LLVM then emits EXTRQ/INSERTQ (66 0F 78), which #UD -> 0xC000001D on Intel CPUs
REM   (confirmed from an end-user crash dump). x86-64-v3 matches the SDK's FFX build.
REM   Stage 2 (ThinLTO) is layered on once Stage 1 is proven.
...
if "%1"=="configure" (
  ...
  cmake -S e:/Repositories/nhl-legacy-recomp -B %BDIR% -G Ninja ^
    ...
    -DCMAKE_C_FLAGS_RELWITHDEBINFO="-O3 -DNDEBUG -march=x86-64-v3" ^
    -DCMAKE_CXX_FLAGS_RELWITHDEBINFO="-O3 -DNDEBUG -march=x86-64-v3" ^
